...
const SYSTEM_INSTRUCTION = `당신은 한국 편입 영어 시험지를 오답 카드(mistake card)로 변환하는 어시스턴트입니다.
...
export async function extractMistakeCards(text: string): Promise<MistakeCard[]> {
  if (!text.trim()) return [];
  ...
  try {
    ...
    return parsed.filter(isMistakeCard);
  } catch {
    return [];
  }
}

function isMistakeCard(v: unknown): v is MistakeCard {
