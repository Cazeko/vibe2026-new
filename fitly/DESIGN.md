...
> v3.0 (2026-05-06) — 컨셉 피벗 (편입 영어 → 초등 임용 1차 + 팟캐스트). 시각 시스템(폰트·색·여백·모션·메모러블 앵커)은 본질적으로 변경 없음. 텍스트(제품 컨텍스트·라벨·금지어·결정 로그)만 갱신.
>
> v3.0.1 (2026-05-06) — 자료 페이지 cut. 사용자 PDF 업로드 동선 폐기. 헌법 제14조 5항 정합.
...
| 축 | 정의 |
|---|---|
| **무엇** | 한국 초등교사 임용시험(1차) 학습 플래너 PWA + NotebookLM 스타일 자동 생성 팟캐스트 (헌법 제2조) |
| **누구** | 사설 학원·인강을 끊을 수 없는 환경의 임용 준비생 — 격차를 자동화·플래닝·시드 분석으로 *완화* |
| **공간** | 태블릿 가로(1024×768) PWA · 데스크톱 함께 지원 · **모바일 1차** (v3.5.3 — 팟캐스트 등 이동 중 학습 시나리오 정합) |
...
1. *사설 인강 광고 같은 인상을 주는가?* → A 라면 거부, B 라면 선택지 (희소 컬러·차분한 톤)
2. *임용 준비생이 첫 5초에 "아, 학습 노트" 라고 인지하는가?* → 인지하면 채택
...
`--color-accent` 는 *오직* 다음 6가지 위치에만 사용한다:
...
6. **로그인/회원가입 랜딩 hero의 잉크 trail** — 만년필이 펀치라인을 *쓰는* 한 순간의 글쓰기 trail. 메모러블 앵커("학습 진척도만 컬러로 빛난다")의 *학습 진입 시작* 시각화 — 사용자가 학습 여정을 시작하는 *첫 잉크* (2026-05-06 brainstorming 합의 + 사용자 명시 승인).
...
#### 4.4.1 영역(domain) 분류 단서 (v3.5.4 신설, 2026-05-12)

본 §4.4 의 "라벨+아이콘+좌측 보더 3축" 원칙 안에서, **좌측 보더의 *hue* 만은 영역(domain)·과목 분류 목적으로 다색 허용**한다. SRS 4등급 시맨틱 (2026-05-07 §4.4 ⓟ 예외) 와 같은 *학습 인지 핵심* 단서로 인정한다. 단 다음 조건을 모두 만족할 때만 적용한다.
...
토큰: `--subject-1` ~ `--subject-7` (cream-friendly 7색 라운드 로빈). 8과목 이상은 같은 토큰을 modulo 로 순환 사용한다. 다크 모드는 광도(L) 만 보정한다.

본 단서는 §4.3 "한 번만 빛난다" 원칙과 *충돌하지 아니한다* — 좌측 3px 보더는 본 메시지가 가리키는 *카드 강조 빛* 이 아니라 영역 *식별자* 이며, 한 화면 안에 동시에 여러 색이 등장해도 정보 분류 목적 안에서 정당화된다.
...
- **UI 학습자 호칭**: **"{사용자명} 선생님"** 으로 고정 (2026-05-12 사용자 명시 결정).
  - 인사말: `안녕하세요, {displayName} 선생님.` — displayName 미설정 시 `안녕하세요, 선생님.` fallback.
  - 마이/설정 영역의 사용자 fallback 라벨: `Fitly 선생님` (이메일이 있으면 이메일 우선 표기).
  - 본 호칭은 헌법 제0조의 "주인님"(Claude ↔ 주관자 대화 호칭)과 분리 사안 — UI는 fitly 학습자(임용 수험생) 호칭. 제38조 7항 사용자 즉시 발화 우선 정합.
- **존댓말**: 모든 시스템 메시지·알림·CTA·empty state 본문은 존댓말. (헌법 제0조의 어투 부분은 그대로 적용)
- 화면 라벨: 간결한 명사형 ("학습 진척도", "오늘의 학습 플랜")
- AI 추천 본문: 제안형 ("...해드릴까요?"), 단정형·강제형 금지
- 알림: 사실 + 영향 (예: "학습 진척도가 +3 올랐습니다. 어제 69 → 오늘 72.")
- **동기부여 카피 (v3.5.3 신설)**: subtitle·empty state·AI 추천 일부에 학습 의지 환기 카피를 *보수적*으로 적용. 가이드 — ① 펀치라인 *"적합도"* 톤 유지 (강제·재촉 인상 금지) ② 사실 + 권유 조합 (예: "오늘의 적합도 한 점, 함께 짚어 드릴게요.") ③ "달려볼까요?" 등 게임풍 표현은 피한다.
...
### 9.4 지역 교육청 라벨 [v3.0 갱신]

지역 교육청을 표시할 때 *합격 데이터를 보유한 것처럼* 보일 수 있는 표현을 사용하지 아니한다. UI 에서 노출 가능한 정보는 공개된 시험일·시험과목·시험시간·시험장소에 한정한다 (헌법 제15조). 지역 교육청 입력은 선택 사항이며, 미입력 시에도 모든 학습 기능이 정상 동작한다 (시드 데이터는 전국 공통).

### 9.5 시드 데이터 라벨

기출분석·풀이·팟캐스트의 모든 콘텐츠 카드에는 다음 메타가 노출된다 (헌법 제18조의2·제27조 정합).

- 출처: `2024학년도 교직논술 1번` 형식의 기출 좌표
- 검증 상태: `verified: true | false` 배지 (검토 미완료는 앰버 색 라벨)
- 생성 방식: AI 모범답안·키워드·팟캐스트는 `AI 생성` 보조 라벨

본 메타는 사용자가 학습 카드의 신뢰 수준을 판단할 수 있게 하는 시각적 안전장치이다.
...
| 2026-05-06 | v3.0 — 컨셉 피벗 (편입 영어 → 초등 임용 1차 + 팟캐스트) 텍스트 갱신 | 헌법 v3.0 정합. 시각 시스템(폰트·색·여백·모션·메모러블 앵커)은 유지, 라벨/금지어/시드 메타만 갱신. 공책 메타포·세리프 헤드라인·`#1F5C4A` 단일 액센트는 임용 컨셉에도 자연스럽게 어울려 보존. |
| 2026-05-06 | 시드 데이터 라벨 정책 신설 (9.5) | 기출 좌표 + verified 배지 + AI 생성 보조 라벨 — 헌법 제18조의2·제27조 정직성 정합. 임용고시 레이아웃 이미지 6의 "verified: false" 배지를 디자인 시스템에 정식 편입. |
| 2026-05-06 | v3.0.1 — 자료 페이지·사용자 PDF 업로드 동선 cut | 시드 1,000+ 카드로 학습 콘텐츠 충분, PDF 변환 마찰이 헌법 제5조 진입장벽 0과 충돌, 사설 학원 모의고사 업로드 시 제27조 저작권 회색지대 — 차별화·마법성 어디에도 속하지 않아 cut. 헌법 제14조 5항·제13조 v3.0.1 정합. |
| 2026-05-06 | §4.3 evergreen 6번째 사용처 추가 — 로그인/회원가입 랜딩 hero의 잉크 trail | brainstorming 합의 + 사용자 명시 승인. 메모러블 앵커의 "학습 진척도만 컬러로 빛난다"가 *학습 진입 시작*에도 확장된다는 해석. main app의 evergreen 5번 한정 정책은 그대로 유지 — 본 추가는 `/login`·`/signup` 한정. 헌법 제16조의2 정합. 디자인 plan: `docs/plans/2026-05-06-login-landing-design.md`. |
| 2026-05-07 | §2 "그라디언트 일체 금지"의 *알파 페이드 마스크* 예외 명시 | `/design-review` audit C007 — 풀이트랙 본문 길이 초과 시 단색 알파 페이드 (`bg-gradient-to-t from-card to-transparent`)는 *시각 장식*이 아닌 *접힘 어포던스*. §2 금지의 진짜 의도는 *컬러 그라디언트* (학원 광고풍 다색 충돌). 단색 알파 페이드는 허용. 사용자 명시 승인. |
| 2026-05-07 | §4.4 시맨틱 운영의 *SRS 등급 4분할* 예외 명시 | `/design-review` audit C016 — 풀이트랙 등급 4버튼(Again·Hard·Good·Easy)의 시맨틱 4색(error·warning·info·evergreen)은 *학습 인지 핵심* (red=다시·amber=어려움·blue=정상·green=쉬움). evergreen이 "Good"에 사용되는 것도 §4.3 한정 외이지만, 본 컨텍스트는 SRS 학습 결정 인지의 일부. 등급 4분할 시맨틱은 §4.4 "라벨+아이콘+좌측 보더 3축" 정신 안에서 *모두 허용*. 사용자 명시 승인. |
